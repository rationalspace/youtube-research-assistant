...
CONFIG = {
    ...
    "min_video_duration": 60,  # Skip videos shorter than this (in seconds) - filters out Shorts
        ...
        try:
            ...
            for video in video_details.get('items', []):
                ...
                if duration_seconds < min_duration:
                    print(f"    ⏭️  Skipping short video ({duration_seconds}s): {video['snippet']['title']}")
                    continue
...
"""
        ...
        # Save summaries to file if new videos found
        if new_videos_found and all_summaries:
            timestamp = datetime.now().strftime('%Y-%m-%d')
            filename = f"daily_summary_{timestamp}.txt"
            
            file_content = f"""Daily YouTube Financial Video Summary
...
"""
            ...
            self.save_to_file(file_content, filename)
        else:
            print("\n📭 No new videos found.")
